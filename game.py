...
def state_summary(state: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "inventory": state["inventory"], "traits": state["traits"],
...
def state_options(state: dict[str, Any], events: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {"ok": True, "message": f"你正在{state['location']}，可执行 explore。",
            "actions": {"explore": True, "return": True}, "state": state_summary(state)}
...
def command_help() -> dict[str, Any]:
    return {"message": "每次运行只执行一个动作，避免多轮 input() 造成输入错位。",
            "commands": ["python game.py status", "python game.py options",
                         ...
                         "python game.py deeper", "python game.py eat",
...
def perform_action(state: dict[str, Any], events: list[dict[str, Any]],
    ...
    if action == "explore":
        ...
        save_state(state)
        return {"ok": True, "message": "遭遇事件", "event": event_payload(state, event),
                "state": state_summary(state)}
    if action == "choose":
        ...
        add_journal(state, f"在{location}遭遇“{event['title']}”，选择{choice_key}：{result}")
        state["pending_event"] = None
        state["run_events"] += 1
        unlocked = None
...
def print_human(payload: dict[str, Any]) -> None:
    ...
    if "event" in payload:
        ...
        for choice in event["choices"]:
            ...
            print(f"{choice['key']}. {choice['text']}{suffix}")
    if "choices" in payload:
        for choice in payload["choices"]:
            print(f"- {choice['action']}: {choice['text']}")
    if "actions" in payload:
        print("可执行：", json.dumps(payload["actions"], ensure_ascii=False))
    ...
    if payload.get("unlocked"):
        print(f"【新地点解锁】{payload['unlocked']}")
...
def main() -> None:
    ...
    if "--json" in args:
        ...
        args.remove("--json")
    if not args:
        payload = {"ok": True, **command_help(), "state": state_summary(state)}
    elif args[0].lower() == "json":
        if len(args) < 2:
            payload = {"ok": False, "message": "缺少JSON指令。", "state": state_summary(state)}
        else:
            try:
                instruction = json.loads(args[1])
                action = str(instruction.get("action", "help"))
                action_args = instruction.get("args", [])
                if "choice" in instruction:
                    action_args = [instruction["choice"]]
                if "location" in instruction:
                    action_args = [instruction["location"]]
                if not isinstance(action_args, list):
                    action_args = [str(action_args)]
                payload = perform_action(state, events, action, [str(x) for x in action_args])
            except (json.JSONDecodeError, TypeError) as exc:
                payload = {"ok": False, "message": f"JSON指令错误：{exc}",
                           "state": state_summary(state)}
        json_output = True
    else:
        payload = perform_action(state, events, args[0], args[1:])
    if json_output:
        print(json.dumps(payload, ensure_ascii=False, indent=2))
    else:
        print("《林间夜行》v0.4 稳定指令版")
        print_human(payload)
