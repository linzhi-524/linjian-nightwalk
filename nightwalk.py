...
def state_summary(state: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "lamp_info": {
            "current": state.get("lamp_oil", 0),
            "effect": "点灯后下一次探索更安全，并有35%概率发现额外线索。",
            "dark_cost": "摸黑探索不耗灯油，但体力-1，并可能错过隐藏细节。",
            "sources": ["小屋补给", "灯油商店", "探索事件", "任务奖励"],
            "shop_price": state.get("lamp_shop_price", 3)
        }
    }
...
def lamp_refill_available(state: dict[str, Any]) -> bool:
    return state["day"] - state.get("last_lamp_refill_day", 0) >= 3

def lamp_guidance(state: dict[str, Any]) -> str:
    oil = state.get("lamp_oil", 0)
    if oil <= 0:
        return ("灯油已用尽。可在林间小屋执行 refill_lamp 领取周期补给，"
                "或执行 buy_lamp 以3金币购买1份；部分事件和任务也会奖励灯油。")
    return (f"当前灯油{oil}份。执行 light 消耗1份，为下一次探索提供安全照明，"
            "并有概率发现额外线索；也可用 explore_dark 摸黑探索，但体力-1。")
    ...
    if state["location"] == "林间小屋":
        return {"ok": True, "message": "林间小屋可执行动作。\n" + lamp_guidance(state),
                "actions": {"travel": state["unlocked_locations"],
                            ...
                            "refill_lamp": lamp_refill_available(state),
                            "buy_lamp": state["coins"] >= state.get("lamp_shop_price", 3),
                            "lamp_help": True,
...
def command_help() -> dict[str, Any]:
    return {"message": "每次运行只执行一个动作，避免多轮 input() 造成输入错位。",
            "commands": ["python game.py status", "python game.py options",
                         ...
                         "python game.py deeper", "python game.py lamp_help",
                         "python game.py refill_lamp",
                         "python game.py buy_lamp",
                         "python game.py light",
...
def perform_action(state: dict[str, Any], events: list[dict[str, Any]],
    ...
    if action == "lamp_help":
        return {"ok": True, "message": lamp_guidance(state), "state": state_summary(state)}

    if action == "refill_lamp":
        if state["location"] != "林间小屋":
            return {"ok": False, "message": "只能在林间小屋领取灯油补给。", "state": state_summary(state)}
        if not lamp_refill_available(state):
            wait_days = 3 - (state["day"] - state.get("last_lamp_refill_day", 0))
            return {"ok": False, "message": f"补给还没准备好，再过{wait_days}夜可领取。", "state": state_summary(state)}
        state["lamp_oil"] += 1
        state["last_lamp_refill_day"] = state["day"]
        add_journal(state, "[林间小屋｜灯油补给] 获得灯油+1。")
        save_state(state)
        return {"ok": True, "message": "你在小屋柜底找到一小瓶灯油。灯油+1。", "state": state_summary(state)}

    if action == "buy_lamp":
        if state["location"] != "林间小屋":
            return {"ok": False, "message": "请先回到林间小屋购买灯油。", "state": state_summary(state)}
        price = state.get("lamp_shop_price", 3)
        if state["coins"] < price:
            return {"ok": False, "message": f"金币不足。购买1份灯油需要{price}金币。", "state": state_summary(state)}
        state["coins"] -= price
        state["lamp_oil"] += 1
        add_journal(state, f"[林间小屋｜灯油商店] 花费{price}金币购买灯油+1。")
        save_state(state)
        return {"ok": True, "message": f"你花费{price}金币购买了1份灯油。", "state": state_summary(state)}
    ...
    if action == "light":
        ...
        save_state(state)
        return {"ok": True, "message": "你点亮旧提灯，灯油-1。下一次探索不会因摸黑损失体力，并有35%概率发现额外线索。",
                "state": state_summary(state)}
    if action == "explore_dark":
        ...
        add_journal(state, f"[{state['location']}｜无灯探索] 为节省灯油摸黑前进，体力-1，且可能错过隐藏细节。")
        if state["hp"] <= 0:
...
def _human_text(payload: dict[str, Any]) -> str:
    ...
    if "state" in payload:
        ...
        if s.get("lamp_info"):
            li = s["lamp_info"]
            lines.append(f"灯油说明：当前{li['current']}份；{li['effect']} {li['dark_cost']}")
            lines.append("获取方式：" + "、".join(li["sources"]) + f"；商店价{li['shop_price']}金币/份")
        if s.get("inventory"):
            lines.append("背包：")
